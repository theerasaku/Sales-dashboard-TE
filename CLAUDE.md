...
---
## สถานะโครงการ
**ล่าสุด: 3.8 เสร็จ (AI Intake อัตโนมัติ · Edge Function) — 🎉 จบ roadmap ครบทุก step! เหลือแค่ 1.7 ทดสอบเครื่องจริง (iPhone/S24/iPad)**
...
🖨️ **พิมพ์ฟอร์มใช้ print stylesheet ห้ามเปลี่ยนไปใช้ library ทำ PDF** — `docs/css/print.css` + `docs/js/ui/formprint.js`
   print.css เป็น**ข้อยกเว้นเดียว**ที่ hardcode สีได้ (`#000`/`#fff` + สีสามเหลี่ยมมุม Book 3 สี) เพราะฟอร์มต้องสีคงที่เสมอแม้ธีมจอเปลี่ยน
   🔺 **Book 3 สี พิมพ์ = สามเหลี่ยมมุมขวาบนตามระดับ** (เขียว/เหลือง/แดง) แทนแถว "สีในสมุด" · ใช้ `@page{margin:0}` + padding บน `.pf-page` (มุมถึงชิดขอบ) · ตอนพิมพ์ต้องตั้ง Margins: None
...
💾 **Backup รวม (3.6) = `adapter.exportAll()` → `buildBackup()` = BACKUP_FORMAT** (import-map.js) · หน้า Admin
   restore เรียงตาม FK · ข้าม profiles/team_access/signoffs (auth/append-only) · กู้สำหรับ "โปรเจกต์เดิม"
🎨 **ธีม (3.7): สีจริงอยู่ใน app.css ที่เดียว** — theme.js แค่สลับ `data-theme`/`data-accent` บน `<html>` (ไม่มี hex ในไฟล์ JS)
   `--accent-soft`/`--accent-text` derive จาก `--accent`+`--text` (color-mix) · เพิ่มธีม = เขียนชุดตัวแปรใต้ `[data-theme="X"]`
   สคริปต์ inline ใน `<head>` ตั้งธีมก่อน CSS วาด (กันจอกระพริบ) — **ห้ามย้ายไปโหลดใน module** ไม่งั้นกระพริบกลับมา
🔑 **AI Intake อัตโนมัติ (3.8): ANTHROPIC_API_KEY อยู่ใน Supabase secrets เท่านั้น** — `supabase/functions/ai-intake/`
   frontend เรียกผ่าน `adapter.aiExtract()` (token อยู่ใน adapter · Edge Function ตรวจ JWT ก่อนเรียก Claude)
   3.8 เปลี่ยนแค่ "JSON มาจากไหน" · staging/preview/merge (3.5) ใช้ต่อทั้งหมด (`stageRecords` ใช้ร่วม 2 ทาง)
   **ไม่ deploy ก็ยังใช้ได้** — ตกไปทางก๊อปคำสั่งวางใน Claude เอง (3.5) ฟรี
