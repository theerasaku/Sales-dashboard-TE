...
docs/tools/
  import-json.html            ← ย้ายข้อมูล JSON v1/v2/v3 เดิม → DB + กู้คืนไฟล์สำรอง
                                 ⚠️ ต้องอยู่ใต้ docs/ เท่านั้น — GitHub Pages เสิร์ฟแค่โฟลเดอร์นี้
                                    ถ้าวางที่ tools/ ระดับ repo จะเปิดจากเว็บไม่ได้
...
---
## สถานะโครงการ
**ล่าสุด: 2.3 เสร็จ (แผนติดต่อลูกค้า + เตือนงานค้างบน dashboard) — ถัดไป 2.4 หน้า Admin + role manager**
...
🔴 **`team_id` ว่าง = ทุกคนที่ไม่ใช่ admin แตะแถวนั้นไม่ได้** (`can_access_team()` คืน false)
   `activities` เติมทีมให้อัตโนมัติแล้ว · **`customers` / `pending_projects` ยังไม่ได้เติม — ต้องแก้ตอน 2.4**
...
_เสร็จแล้ว: 0.1 · 0.2 · 1.1–1.6 · 2.1 · 2.2 · 2.3 — ยังเหลือ 12 step (1.7 เหลือทดสอบเครื่องจริง)_
