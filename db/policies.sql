...
-- ══════════════════════════════════════════════════════════
-- สิทธิ์ระดับตาราง (GRANT) — คนละชั้นกับ RLS ต้องมีครบทั้งคู่
--
--   GRANT = "แตะตารางนี้ได้ไหม"      (ถ้าไม่มี → error 42501 permission denied)
--   RLS   = "แตะได้แล้วเห็นแถวไหน"   (ถ้าไม่มี policy → ไม่เห็นแถวเลย)
--
-- ⚠️ อย่าตัดบล็อกนี้ทิ้ง: project นี้ไม่ได้ GRANT ให้ authenticated มาให้อัตโนมัติ
--    (เช็กแล้วหลังรัน schema.sql ทั้ง 5 ตารางตอบ 42501)
--    ถ้าไม่มีบรรทัดพวกนี้ ล็อกอินสำเร็จแล้วก็ยังอ่านข้อมูลไม่ได้อยู่ดี
-- ══════════════════════════════════════════════════════════

grant usage on schema public to anon, authenticated;

-- ผู้ใช้ที่ล็อกอินแล้ว: แตะได้ทั้ง 5 ตาราง — แต่เห็น/แก้ได้เฉพาะแถวที่ policy ข้างบนอนุญาต
grant select, insert, update, delete
  on teams, profiles, pending_projects, follow_logs, project_contacts
  to authenticated;
