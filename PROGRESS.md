...
| 1.1 | schema.sql + policies.sql + seed.sql (B1, B2) | M | 🔄 รัน schema + policies แล้ว · เหลือ seed + ตั้ง admin |
...
## ขั้นตอนต่อไป — ต้องให้ผู้ใช้ทำ

1. รัน `seed.sql` ใน SQL Editor (ทีม 5 ทีม)
2. Authentication → Sign In/Providers → Email → **ปิด "Enable sign ups"**
3. Authentication → Users → **Invite user** → อีเมลตัวเอง → กดลิงก์ในเมลตั้งรหัสผ่าน
4. SQL Editor: `update profiles set role='admin', full_name='เก๋' where email='<อีเมลคุณ>';`
5. ระบุขอบเขตงานของทีม **GOV.3** (ตอนนี้ description ยังว่าง)
